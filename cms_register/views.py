...
import collections, re, subprocess, json, urllib, jdatetime, locale
...
def register(request, x = 0):
    ...
    if request.method == 'POST':
        ...
        if ok and not x:
            ...
            user.save()
            res = subprocess.call(['cmsAddUser', '-p', info['password'], '-e', info['email'], info['name'], info['lname'], info['username']])
            if not res:
                subprocess.call(['python', 'python/EditUser.py', '-p', info['password'], '-e', info['email'], '-fn', info['name'], '-ln',info['lname'], info['username']])
            done = True
        if ok and x:
            ...
            if info['password'] != '':
                ...
                subprocess.call(['python', 'python/EditUser.py', '-e', info['email'], '-fn', info['name'], '-ln',info['lname'], info['username']])
            user.save()
            done = True
    info['password'] = info['password2'] = ''
    return render(request, "cms_register/register.html", { 'RECAPTCHA_PUBLIC_KEY' : settings.RECAPTCHA_PUBLIC_KEY, 'ok' : done, 'info' : info, 'error' : error, 'place' : place,  'type' : types, 'caper' : cap_error, 'x' : x })
...
def contest_view(request):
    ...
    return render(request, "cms_register/contests.html", { 'contests' : clist, 'date' : date, 'time' : time, 'expired' : expired, 'registered' : registered, 'done' : done , 'dur' : dur , 'need' : need, 'ended' : ended, 'cdown' : cdown })
...
def ranking(request, contest_id, unof = False):
	...
	return render(request, "cms_register/ranking.html", { 'users' : tab, 'head' : head , 'mx' : mx, 'exp' : expired , 'error' : False, 'contest' : contest, 'un' : unof})
